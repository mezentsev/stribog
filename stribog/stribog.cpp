#include "stribog.h"
// Итерационные константы, разбиты для удобства
byte C_r[12][64] = {
            {
...
void xor512(byte *a, byte *b, byte *result)
{
    for(int i = 0; i < 64; ++i)
    {
        result[i] = a[i] ^ b[i];
...
void mod512(byte *a, byte *b, byte *result)
{
    int i = 0, t = 0;
    for(i = 63; i >= 0; --i)
    {
...
void P(byte *result)
{
    ...
    for(int i = 0; i < 64; ++i)
    {
        repl[i] = result[tau_r[i]];
    }

    memcpy(result, repl, 64);
}
...
// E(K, ) = X[K13]LPSX[K12] ... LPSX[K2]LPSX[K1](m)
void E(byte *K, byte *m, byte *result)
{
    xor512(m, K, result);
    ...
    for(int i = 0; i < 12; ++i)
    {
        ...
        L(result);
        GenK(K, i);
        xor512(result, K, result);
    }
}

// Функция сжатия
void g(byte *N, byte *h, byte *m)
{
    byte result[64], r[64];
    xor512(h,N,result);
    S(result);
...
void hash(byte *message, byte *out, int len)
{
    // Инициализация
    byte N[64]      = {
    ...
    // Этап 2
    // Проверка на длину и деление
    ubyte length = strlen((char*)message);
    ...
    // 3.1
    memset(m,0,64);
    memcpy(m + 63 - length + ( (length << 3 & 0x7) == 0 ), message, length + 1 - ( (length << 3 & 0x7) == 0 ));
    ...
    // 3.3
    // Получение N
    ubyte length_b = (length << 3);

    VEC512[63] = (length_b >> 0) & 0xFF; // 256
    VEC512[62] = (length_b >> 8) & 0xFF; // 512
    mod512(N, VEC512, N);
